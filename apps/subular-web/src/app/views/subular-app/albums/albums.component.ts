...
import { IAlbum, IArtist } from '@Subular/core';
import { Observable } from 'rxjs/Observable';
import {
  RouterResolverDataObservable,
  RouterParamObservable
} from '@Subular/core';
import { SubsonicCachedService, SubsonicService } from '@Subular/core';
...
export class AlbumsComponent implements OnInit {
  ...
  constructor(
    ...
    private cached: SubsonicCachedService,
    private subsonic: SubsonicService
  ) {}
  ngOnInit() {
    this.artist$ = RouterParamObservable<number>(
      this.route,
      this.router,
      'artistId'
    ).switchMap(artistId => this.cached.getArtistById(artistId));
    this.albums$ = RouterResolverDataObservable<IAlbum[]>(
      this.route,
      this.router,
      'albums'
    );
