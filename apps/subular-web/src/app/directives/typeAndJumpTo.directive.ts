import { Directive, ElementRef, EventEmitter, Input, HostListener, AfterViewInit, Output } from '@angular/core';
...
export class TypeAndJumpToDirective implements AfterViewInit {
	...
	scrollToItem(listItem: any) {
		if (listItem) {
			...
			const artistLI = nativeHtmlUL.querySelector(`[class='${listItem.id}']`) as HTMLLIElement;
	...
	getFirstArtistThatStartsWith(startsWith: string): any {
		return this.jumpTo.find(value => value.name
			.toLowerCase()
			.replace('the ', '')
